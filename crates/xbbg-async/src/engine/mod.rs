...
pub use config::*;
use request_params::merge_raw_kwargs_into_elements;
pub use request_params::RequestParams;
pub use request_pool::RequestWorkerPool;
pub(crate) use startup::start_configured_session;
#[cfg(test)]
...
impl Engine {
    ...
    pub fn start(config: EngineConfig) -> Result<Self, BlpAsyncError> {
        ...
        let (request_pool, subscription_pool, rt) =
            startup::build_engine_components(config.clone())?;
